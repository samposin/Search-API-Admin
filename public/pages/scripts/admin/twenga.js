		    ...
		    */
			...
			grid.init({
				...
				dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options
					...
					"ajax": {
						"url": url_twenga_show_ajax, // ajax source
						"method": "post",
						"dataType": 'json',
...
var get_search_clicks_ajax =function(){
	 ...
	 */
	var url=url_twenga_download_ajax+month1+api1+publisher1;

	$('#csv_download').attr('href',url);
	$.ajax({
		type:"post",
		url:url_twenga_show_ajax,
		dataType:"json",
		data:{
			"search_api":api,
			"_token":csrf_token,
...
var  generate_daily_report_twenga=function(){

	$.ajax({
		type:"post",
		url:url_twenga_generate_daily_report_ajax,
		dataType:"json",
		headers: {'X-CSRF-TOKEN': csrf_token},
		data:{
		},
		...
		success:function(data) {
			...
			if(data['success']=='1'){
				...
				html="<br><br><div class='alert alert-success'>"+data['message']+"</div>";
			...
			if(data['success']=='0'){
				...
				html="<br><br><div class='alert alert-danger'>"+data['message']+"</div>";

			}

			$("#message-daily-report").fadeIn();
			$("#message-daily-report").html(html).fadeOut(5000);
		}
	});
}
jQuery(document).ready(function(){
	...
	$('#btn_generate_daily_report').click(function(){
      generate_daily_report_twenga();
	...
	$('#btn_email_daily_report').click(function() {
		...
		if(email!='') {
			...
			$.ajax({
				type: "post",
				url: url_twenga_email_send_ajax,
				dataType: "json",
				data: {
					"email": email,
					"api": api,
